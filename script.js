    ...
    input.onchange = e => {
        ...
        reader.onload = event => {
            try {
                ...
                // Mettre à jour la liste pour l'input
                const productsList = document.getElementById("products-list");
                productsList.innerHTML = "";
                productsJSON.forEach(p => {
                    const option = document.createElement("option");
                    option.value = p.name; // Utilisé pour rechercher le produit
...
posProduct.addEventListener("input", () => {
    ...
    const productDetailsEl = document.getElementById("product-details"); // créer ce div dans ton HTML sous le champ produit
  ...
  productDetailsEl.innerHTML = `
    ...
    <p><strong>PRICE SELL:</strong> ${product.price.toFixed(2)} DH</p>
    <p><strong>PRICE UNIT:</strong> ${product.priceUnit}</p>
    <p><strong>BOX UNIT:</strong> ${product.boxUnit}</p>
    <p><strong>BOX PRICE:</strong> ${product.boxPrice}</p>
    <p><strong>PROFIT:</strong> ${product.profit}</p>
    <p><strong>STOCK:</strong> ${product.currentStock}</p>
    <p><strong>SOLD STOCK:</strong> ${product.soldStock}</p>
    <p><strong>DESCRIPTION:</strong> ${product.description}</p>
    <p><strong>EXPIRATION:</strong> ${product.expiration}</p>
`;
