...
const saleLines = cart.map(item => ({
    ...
    payment_mode: type,
    payment_status: type==="cash" ? "payé" : "non payé", // <-- ici
    createdAt: now
}));
...
function updateCreditStatus(index, value){
    credits[index].payment_status = value;

    // Si le crédit devient payé, on peut éventuellement le déplacer dans les ventes
    if(value === "payé"){
        const paidCredit = credits.splice(index, 1)[0];
        sales.push(paidCredit);
        renderSalesTable();
    }

    renderCreditsTable();
    downloadJSON("credits.json", credits); // sauvegarder automatiquement
    downloadJSON("ventes.json", sales);
}
