...
// CART FUNCTIONS
// ============================
let sortCartDirection = {}; // mémorise asc / desc pour chaque colonne
...
function renderCart() {
    ...
    cart.forEach((item, index) => {
        ...
        row.innerHTML = `
            ...
            <td>${item.discount.toFixed(2)} DH</td>
    ...
    cartTotalEl.textContent = total.toFixed(2) + " DH";
}

// Trier le panier
function sortCart(column) {
    // bascule entre asc et desc
    if (!sortCartDirection[column]) sortCartDirection[column] = "asc";
    else sortCartDirection[column] = sortCartDirection[column] === "asc" ? "desc" : "asc";

    cart.sort((a, b) => {
        let valA = column === "total" ? a.price * a.qty - a.discount : a[column];
        let valB = column === "total" ? b.price * b.qty - b.discount : b[column];
        ...
        if (valA > valB) return sortCartDirection[column] === "asc" ? 1 : -1;
        if (valA < valB) return sortCartDirection[column] === "asc" ? -1 : 1;
        return 0;
    });
...
posClient.addEventListener("input", () => {
    ...
    if (!value) {
        ...
        posClient.value = "Client passager";
        clientAutocomplete.style.display = "none";
    ...
    if(matches.length === 1){
        posClient.value = matches[0].name;
        clientAutocomplete.style.display = "none";
        nextStepBtn.focus(); 
    } else {
        matches.forEach(c => {
...
document.getElementById("next-step2").addEventListener("click", function () {
    ...
    if(client === "" || client.toLowerCase() === "client passager"){
        client = "Client passager";
        posClient.value = client;
    }
