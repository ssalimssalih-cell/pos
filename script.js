        ...
        reader.onload = event => {
            try {
                ...
                if (Array.isArray(importedProducts)) {
                    productsJSON.splice(0, productsJSON.length, ...importedProducts);
                ...
                } else alert("Fichier produits invalide !");
            } catch (err) {
                alert("Erreur lors de l'importation des produits : " + err.message);
            }
        };
        reader.readAsText(file);
    };
...
posProduct.addEventListener("input", () => {
    ...
    if(value && matches.length === 1){
        ...
        clientAutocomplete.style.display = "none"; // on peut afficher la liste si nécessaire
...
function renderCart() {
    ...
    cart.forEach((item,index)=>{
        ...
        row.innerHTML = `
            ...
            <td>${item.discount.toFixed(2)} MAD</td>
            <td>${lineTotal.toFixed(2)} MAD</td>
            <td>
...
saveSaleBtn.addEventListener("click", () => {
    ...
    const saleLines = cart.map(item => ({
        id_vente: saleId,
        client, // <-- ici on utilise la variable client déjà corrigée
    ...
    if(matches.length === 1){
        ...
        matches.forEach(p => {
            ...
            item.addEventListener("click", () => {
                ...
                infoBrand.textContent = p.brand;
                infoPrice.textContent = p.priceSell;
                infoStock.textContent = p.currentStock;
                infoDesc.textContent = p.description;
                infoExp.textContent = p.expiration;
                productAutocomplete.style.display = "none";
                posQty.focus();
            });
            productAutocomplete.appendChild(item);
        });
        productAutocomplete.style.display = matches.length ? "block" : "none";
    }
});
...
creditsFileInput.addEventListener("change", function () {
    ...
    reader.onload = function (e) {
        try {
            ...
            alert("Credits importés avec succès !");
        } catch (error) {
            alert("Erreur dans le fichier credits.json");
        }
    };
